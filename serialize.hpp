...
std::enable_if_t<is_reflectable_v<T>, std::string> _to_json_impl(const T& obj)
{
    std::string json = "{";
    bool first = true;
    for_each_member(const_cast<T&>(obj), [&](const char* name, const auto& member) {
...
template<typename T>
std::enable_if_t<!std::is_arithmetic_v<T> && !is_reflectable_v<T> && !std::is_same_v<T, std::string>, std::string> _to_json_impl(const T& value)
{
    return "undefined";
}
